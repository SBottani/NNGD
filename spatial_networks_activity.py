...
animation = False # wheter to generate activity on map movie
obstacles = True # set to True for simulation with elevated obstacles

print "\n###################\n Runtime options\n------------------"
print "Plot of the connectivity distribution : {0}".format(plot_distribution)
...
print "Generate activity on map movie  : {0}".format(animation)
...
Lonardoni_excitatory_params = {
    ...
    'V_reset': -58., 't_ref': 2., 'g_L': 12., 'C_m': 281., 'I_e': 150.
}

Lonardoni_inhibitory_params = {
    'a': 2.,'E_L': -70., 'V_th': -50., 'b': 0., 'tau_w': 30.,
    'V_reset': -58., 't_ref': 2., 'g_L': 10., 'C_m': 200., 'I_e': 150.
}
...
## Synpases
synaptic_weigth = 50. #set default synaptic weigth
...
def activity_simulation(net,pop,sim_duration=sim_duration):
    ...
    if nngt.get_config('with_nest'):
        ...
        if nngt.get_config('with_plot'):
            ...
            ns.plot_activity(recorders, records, network=net, show=True,hist=True)
...
def output_graphs(net,set1,set2,restrict):
  ...
  Graphs
  Input :
  - net : neturon population and network
  - set neurons 1 
  - set neurons 2
  ...
  if plot_distribution is True :
      ...
      nngt.plot.degree_distribution(
          net, ["in", "out"], num_bins='bayes', nodes=set2, show=True)
...
def with_obstacles(shape,params = {"height": 250., "width": 250.},filling_fraction = 0.4):
    ...
    ''' Create a Spatial nettwork and seed neurons on top/bottom areas '''
    ...
    # Graphs output
    output_graphs(net,top_neurons,bottom_neurons,
...
def no_obstacles(shape):
  ...
  print ("\n Total number of neurons".format(num_neurons))
  ...
  print "\n----------\n Connectivity"
  print "Connectivity characteristic distance {0}".format(connectivity_scale)
  ...
  for name, area in shape.default_areas.items():  
      ...
      nngt.generation.connect_nodes(net, excitatory_neurons, excitatory_neurons,
                                    "distance_rule", scale=connectivity_scale,
